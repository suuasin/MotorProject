...
namespace SmartAPS.UserLibrary.Helper
{
    public class StringHelper
    {
        ...
        {
            if (StringHelper.IsEmptyID(yn))
                return defaultValue;

            if (string.Equals(StringHelper.Trim(yn), "Y", StringComparison.CurrentCultureIgnoreCase))
                return true;
            else
                return false;
        }
        ...
        public static string ConvertWithQuotes(string str)
        {
            string[] values = str.Replace(" ", "").Split(',');

            List<string> results = new List<string>();
            foreach(string value in values)
                results.Add(string.Format("'{0}'", value));
        ...
        public static List<string> WithQuote(string[] items, char quote)
        {
            List<string> results = new List<string>();
            for (int i = 0; i < items.Length; i++)
                results.Add(WithQuote(items[i], quote));

            return results;
        }

        public static string WithQuote(string item, char quote)
        {
            return string.Format("{0}{1}{2}", quote, item, quote);
        } 
