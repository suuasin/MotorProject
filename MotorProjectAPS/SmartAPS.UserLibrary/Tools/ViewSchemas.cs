...
namespace Template.UserInterface.Tools
{
    public partial class ViewSchemas : XtraUserControlView
    {
            ...
            bool allowNull, bool isPrimaryKey, bool hidden, string caption, string description)
        {
            ...
            if(allowNull)
                row[dt.Columns["ALLOW_NULL"]] = StringHelper.ToStringYN(allowNull);

            if (isPrimaryKey)
                row[dt.Columns["IS_PRIMARY_KEY"]] = StringHelper.ToStringYN(isPrimaryKey);

            if (hidden)
                row[dt.Columns["HIDDEN"]] = StringHelper.ToStringYN(hidden);
        ...
        {
            GridExporter.ExportToExcel(gridView);
        }
    }
}
