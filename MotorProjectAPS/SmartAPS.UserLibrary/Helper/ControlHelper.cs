...
{    
    ...
    {
        public static void BestFitCoulmns(GridView view, int maxWidth = int.MaxValue)
        {
            view.BestFitColumns();
        ...
        public static void SetFilter(GridView gridView, string columnName, string value, AutoFilterCondition condition)
        {
            if (string.IsNullOrEmpty(value) == true)
                return;

            if (gridView.Columns.Any(x => x.FieldName == columnName) == false)
                return;

            gridView.SetAutoFilterValue(gridView.Columns[columnName], value, condition);
        }

        public static RepositoryItemComboBox SetComboBoxColumnType(GridView view, string columnName, params string[] values)
        {
            if (values == null)
                return null;
        ...
        public static RepositoryItemCheckEdit SetCheckBoxColumnType(GridView view, string columnName)
        {
            if (view.Columns.Any(x => x.FieldName == columnName) == false)
                return null;

            RepositoryItemCheckEdit _riEditor = new RepositoryItemCheckEdit();
            _riEditor.ValueChecked = true;
            _riEditor.ValueChecked = "Y";
            _riEditor.ValueUnchecked = "N";

            view.Columns[columnName].ColumnEdit = _riEditor;

            view.OptionsBehavior.EditorShowMode = EditorShowMode.MouseDown;
            return _riEditor;
        }

        public static RepositoryItem SetComboBoxColumnType(GridView view, string columnName, RepositoryItem item)
        {
            if (view.Columns.Any(x => x.FieldName == columnName) == false)
                return null;

            view.Columns[columnName].ColumnEdit = item;

            return item;
        }

        public static void DisableColumnSort(GridView view)
        {
            foreach (GridColumn column in view.Columns)
                column.OptionsColumn.AllowSort = DefaultBoolean.False;
        }

        public static void EnableColumnSort(GridView view)
        {
            foreach (GridColumn column in view.Columns)
                column.OptionsColumn.AllowSort = DefaultBoolean.True;
        }

        public static void DisableEditColumn(GridView view, params string[] columnNames)
        {
        ...
        public static void EnableEditColumn(GridView view, params string[] columnNames)
        {
            foreach (string columnName in columnNames)
            {
                if (view.Columns.Any(x => x.FieldName != columnName) == false)
    ...
    public class PivotGridHelper
    {
        ...
        public static void ExportToExcel(PivotGridControl pivotGrid, string docName)
        {
            ...
            if (dialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (MessageBox.Show("Open the Excel file?", "Open", MessageBoxButtons.OKCancel) == DialogResult.OK)
                {
	...
	//            {
	//                case "htm":
	//                    grid.ExportToHtml(fileName);
	//                    break;
	//                case "mht":
	//                    grid.ExportToMht(fileName);
	//                    break;
	//                case "pdf":
	...
	//        try
	//        {
	//            var progressForm = new Template.UserInterface.UIComponentFactory.ExportProgressDialog(control as Form);
	//            progressForm.Show();
